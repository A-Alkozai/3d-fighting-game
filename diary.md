<table>
    <tr>
        <td>
            <h1>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <br>
                FYP Diary
            </h1>
        </td>
        <td>
            <img src="https://media4.giphy.com/media/v1.Y2lkPTZjMDliOTUyaWpjYzk5MHJ2NHd3aGhoaDM2ZTl4YnQ2eXJpczZ6bDhsYWdoYWM5NCZlcD12MV9naWZzX3NlYXJjaCZjdD1n/xCej66aMjFBgqgWtQV/source.gif" width="220" height="140">
        </td>
    </tr>
</table>

<table>
    <tr>
        <td>
            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbDB4Z2RrYTB1OTZ5Ym1hdWZ0aXV6b2QyZzZtMmp2OGsxd3NhczRqbyZlcD12MV9zdGlja2Vyc19zZWFyY2gmY3Q9cw/fqIOmNAvFOaiQO9GFy/giphy.gif" width="80" height="80">
        </td>
        <td>
            <h2>
                <br>
                Week 1: &nbsp; 29/09/25 - 03/10/25
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </h2>
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Experimenting with animations in Unity
            <br>&nbsp;
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <img src="https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExNnJ0aW5kdjRkZ3Q3cGQ3Nnk2OXd4M2loMjl5Y2tnbXhmd2tpaG4weiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/3ayXJsUrnqci8Nh5J7/giphy.gif" width="200" height="120">
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Started Project Plan
            <br>&nbsp;
        </td>
    </tr>
</table>

<table>
    <tr>
        <td>
            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbDB4Z2RrYTB1OTZ5Ym1hdWZ0aXV6b2QyZzZtMmp2OGsxd3NhczRqbyZlcD12MV9zdGlja2Vyc19zZWFyY2gmY3Q9cw/fqIOmNAvFOaiQO9GFy/giphy.gif" width="80" height="80">
        </td>
        <td>
            <h2>
                <br>
                Week 2: &nbsp; 06/10/25 - 10/10/25
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </h2>
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Completed and submitted project plan
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Researching player state handling
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; I will use bitmasking to represent states
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; I will make an Enum to represent player direction
            <br>&nbsp;
        </td>
    </tr>
</table>

<table>
    <tr>
        <td>
            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbDB4Z2RrYTB1OTZ5Ym1hdWZ0aXV6b2QyZzZtMmp2OGsxd3NhczRqbyZlcD12MV9zdGlja2Vyc19zZWFyY2gmY3Q9cw/fqIOmNAvFOaiQO9GFy/giphy.gif" width="80" height="80">
        </td>
        <td>
            <h2>
                <br>
                Week 3: &nbsp; 13/10/25 - 17/10/25
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </h2>
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Working on a basic UML diagram of the game logic, containing:
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; State Management
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Input Management - With AI and Networking considered
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Combo Management - Creating and matching input --> combo
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Player - How a player will manage their own logic
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; UML design is based off my research I'm currently doing
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Researching input handling/buffering
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Will make a list of inputs, which will act as a queue
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Will set Time-to-Live per input
            <br>&nbsp;
        </td>
    </tr>
</table>

<table>
    <tr>
        <td>
            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbDB4Z2RrYTB1OTZ5Ym1hdWZ0aXV6b2QyZzZtMmp2OGsxd3NhczRqbyZlcD12MV9zdGlja2Vyc19zZWFyY2gmY3Q9cw/fqIOmNAvFOaiQO9GFy/giphy.gif" width="80" height="80">
        </td>
        <td>
            <h2>
                <br>
                Week 4: &nbsp; 20/10/25 - 24/10/25
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </h2>
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Finished my UML diagram for the essential systems
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Preparing to start my input system
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Currently setting up my GitLab and OneDrive
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Planning how I will manage my Assets (fully or partially integrate to Cloud)
            <br>&nbsp;
        </td>
    </tr>
</table>

<table>
    <tr>
        <td>
            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbDB4Z2RrYTB1OTZ5Ym1hdWZ0aXV6b2QyZzZtMmp2OGsxd3NhczRqbyZlcD12MV9zdGlja2Vyc19zZWFyY2gmY3Q9cw/fqIOmNAvFOaiQO9GFy/giphy.gif" width="80" height="80">
        </td>
        <td>
            <h2>
                <br>
                Week 5: &nbsp; 27/10/25 - 31/10/25
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </h2>
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Created an input system for the game
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Inputs are detected (based off the keybind config)
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Inputs are put into an Input Buffer
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Input Buffer automatically removes inputs after a certain frame duration
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Created a recent inputs UI for debugging (player debug settings in future)
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Displays the most recent inputs on the screen
            <br>&nbsp;
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <img src="https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExN2NqcjB6b3V4MXZ5b2UxMDEyZW4wZHJqOGl4YWhxczN1MzFlczB3aCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/auaM7ajGtJAkyCV1TY/giphy.gif" width="200" height="120">
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Working on player status/state system now
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Figuring how the states will interact with one another
            <br>&nbsp;
        </td>
    </tr>
</table>

<table>
    <tr>
        <td>
            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbDB4Z2RrYTB1OTZ5Ym1hdWZ0aXV6b2QyZzZtMmp2OGsxd3NhczRqbyZlcD12MV9zdGlja2Vyc19zZWFyY2gmY3Q9cw/fqIOmNAvFOaiQO9GFy/giphy.gif" width="80" height="80">
        </td>
        <td>
            <h2>
                <br>
                Week 6: &nbsp; 03/11/25 - 07/11/25
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </h2>
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Created a state/status system for the game
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Each state has a state rule
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; State rules consist of Blocks, Overrides and Next-State rules.
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Working on input interpreter and moves system now
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Must determine how strings of input will be interpreted
            <br>&nbsp;
        </td>
    </tr>
</table>

<table>
    <tr>
        <td>
            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbDB4Z2RrYTB1OTZ5Ym1hdWZ0aXV6b2QyZzZtMmp2OGsxd3NhczRqbyZlcD12MV9zdGlja2Vyc19zZWFyY2gmY3Q9cw/fqIOmNAvFOaiQO9GFy/giphy.gif" width="80" height="80">
        </td>
        <td>
            <h2>
                <br>
                Week 7: &nbsp; 10/11/25 - 14/11/25
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </h2>
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Created move system for the game
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; A move.json contains all general move data
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; A move tree is created via this json (node = input command)
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Each node contains move data determined by the sequence of input nodes compared to each moves input sequence
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Still working on input interpreter
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; The logic behind how this will work is difficult, had to keep going back and forth to change the tree
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; By next week I should have:
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Interpreter completed
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; A State UI for debugging
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Animation system started
            <br>&nbsp;
        </td>
    </tr>
</table>

<table>
    <tr>
        <td>
            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbDB4Z2RrYTB1OTZ5Ym1hdWZ0aXV6b2QyZzZtMmp2OGsxd3NhczRqbyZlcD12MV9zdGlja2Vyc19zZWFyY2gmY3Q9cw/fqIOmNAvFOaiQO9GFy/giphy.gif" width="80" height="80">
        </td>
        <td>
            <h2>
                <br>
                Week 8: &nbsp; 17/11/25 - 21/11/25
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </h2>
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Finished Input Interpreter
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Inputs are matched to correct move
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Chained inputs are supported
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Held down inputs are displayed in Recent Inputs UI
            <br>&nbsp;
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <img src="https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExMTFha2dqczVua3RhbHZkZmhkNnI1MGIwdDVlbDhpYWQ3OThwMm9lNCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/5QoE9aV8ZXnXcuh9aK/giphy.gif" width="200" height="120">
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Hardware Limitation - Keyboard
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Differentiating between a single tap and a hold down produces significant delay
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Keys that can be held down will be delayed -> not good for frame sensitive game
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Cannot be avoided sadly -> will try work around it
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Single tap key = 15-30 frames of input
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Holding down key = 50+ frames of input
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Due to increased workload from other modules, work on FYP will be slowed down
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Hoping to at least start the animation system
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; May work on state UI
            <br>&nbsp;
        </td>
    </tr>
</table>

<table>
    <tr>
        <td>
            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbDB4Z2RrYTB1OTZ5Ym1hdWZ0aXV6b2QyZzZtMmp2OGsxd3NhczRqbyZlcD12MV9zdGlja2Vyc19zZWFyY2gmY3Q9cw/fqIOmNAvFOaiQO9GFy/giphy.gif" width="80" height="80">
        </td>
        <td>
            <h2>
                <br>
                Week 9: &nbsp; 24/11/25 - 28/11/25
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </h2>
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Planned Animation System through research
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Have just started implementation
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ◇ &nbsp; Using Mixamo as my source for animations
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Realised my game FPS is not limited and is running at 300+ fps
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Negatively affects performance and causes inconsistency
            <br>&nbsp;
        </td>
    </tr>
</table>

<table>
    <tr>
        <td>
            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbDB4Z2RrYTB1OTZ5Ym1hdWZ0aXV6b2QyZzZtMmp2OGsxd3NhczRqbyZlcD12MV9zdGlja2Vyc19zZWFyY2gmY3Q9cw/fqIOmNAvFOaiQO9GFy/giphy.gif" width="80" height="80">
        </td>
        <td>
            <h2>
                <br>
                Week 10: &nbsp; 01/12/25 - 05/12/25
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </h2>
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Completed the Animation System
            <br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <img src="https://media3.giphy.com/media/v1.Y2lkPTc5MGI3NjExa2x1em1rZnV6MHBjNWg5d3I5Mm5qaHUzZ252NnB0MTh5dDVlOHJtZyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/60FE3znIAmTeU9JoqQ/giphy.gif" width="200" height="120">
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Realised I must refactor some hardcoded data in Animation Json (e.g Total Frames)
            <br>&nbsp;
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Submitted FYP Presentation
            <br>&nbsp;
        </td>
    </tr>
</table>

<table>
    <tr>
        <td>
            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbDB4Z2RrYTB1OTZ5Ym1hdWZ0aXV6b2QyZzZtMmp2OGsxd3NhczRqbyZlcD12MV9zdGlja2Vyc19zZWFyY2gmY3Q9cw/fqIOmNAvFOaiQO9GFy/giphy.gif" width="80" height="80">
        </td>
        <td>
            <h2>
                <br>
                Week 11: &nbsp; 08/12/25 - 12/12/25
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </h2>
        </td>
    </tr>
    <tr></tr>
    <tr>
        <td colspan="2">
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Completed the retrospective section of report
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Working on my interim report now
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ➤ &nbsp;&nbsp; Uploaded my demo video
            <br>&nbsp;
        </td>
    </tr>
</table>

Term 2 FYP Diary

Week 1:   19/01/26 - 23/01/26
      ➤    Refactored the animation timing system to remove hardcoded data.
             ◇   Problem: Logic and view were desyncing because hardcoded JSON frame counts didn't perfectly match actual Unity animation clip lengths.
             ◇   Solution: Created an initialization step that dynamically reads the .anim clip length at runtime and calculates exact logic frames, ensuring perfect deterministic sync.

Week 2:   26/01/26 - 30/01/26
      ➤    Designed and integrated the core Movement System.
             ◇   Implemented MovementData schemas and a dictionary database to store movement speeds and properties.
      ➤    Hooked the movement executor into the Player and Moves system to apply physical displacement alongside animation playback.

Week 3:   02/02/26 - 06/02/26
      ➤    Developed a modular Camera System for combat.
             ◇   Created a CameraMode interface to allow seamless switching between different camera behaviors.
      ➤    Implemented the default combat camera with adjustable positioning, ensuring both players remain in view regardless of distance.


